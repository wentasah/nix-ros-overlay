...
# if there are no updates or updates in just a few root packages in
# the dependency tree.
if ! rush --keep-order --verbose -- bash -c '$(nix-build -A rosPackages.{}.updateAmentVendor) 2>&1' <<<"$update_order"
then
    ...
    for i in $update_order; do
        printf "\nBuilding update script for $i\n\n"
        $(nix-build -A rosPackages.$i.updateAmentVendor)
    done
fi
